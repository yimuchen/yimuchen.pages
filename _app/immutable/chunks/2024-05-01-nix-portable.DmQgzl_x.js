...
that you might want for a prolonged coding experience.`,le,_,tt=`While there is merit in understanding how to use vanilla tool-kits of the
...
modern tools with you to the older machines in a way such that:`,ie,C,st=`<li>You can have access to newer tools not installed by default on the older
...
the specific machines (like job submissions/dataset look up)</li>`,re,d,nt='<a aria-hidden="true" tabindex="-1" href="#some-attempted-solutions"><span class="icon icon-link"></span></a>Some attempted solutions',ce,D,at=`I’ve been trying to solve this problem for the past 8 years, and never quite
got a satisfactory solution:`,pe,I,ot=`<li><strong>dotfile management</strong>: the obvious answer of needing tools to be up-to-date:
...
want to use when developing.</li>`,he,q,lt=`<li><strong>Compiling on remote server</strong>: since tools are all open-source, can we
...
change again, making this solution very fragile.</li>`,de,A,it=`<li><p><strong>All local development</strong>: this was the solution that I used for the longest
...
compatible).</p></li>`,ue,P,rt=`Ultimately, the solution that I am looking for is a distribution agnostic
...
exceeds what I was hoping for.`,me,u,ct='<a aria-hidden="true" tabindex="-1" href="#nix-as-a-portable-package-manager"><span class="icon icon-link"></span></a><code>nix</code> as a portable package manager',ye,B,pt=`The <a href="https://nixos.org/" rel="nofollow"><code>Nix</code> package manager</a> (and its accompanying Linux distribution
...
required libraries are exposed to the package of interest.`,fe,j,ht=`This is the ultimate goal of nix, but for me, the key part of this setup is
...
<code>nix</code> in your own environment.`,ve,m,dt='<a aria-hidden="true" tabindex="-1" href="#nix-running-without-root"><span class="icon icon-link"></span></a>nix running without root',ge,S,ut=`First we need to get a “static” version of the <code>nix</code> package manager
...
nix updates as well as the machine that you are using:`,we,y,mt=`<div class="code-block-header"><span class="code-block-lang">[bash]</span></div> <div class="code-block code-copyable"><pre class="shiki monokai" style="background-color:#272822;color:#F8F8F2" tabindex="0"><code><span class="line"><span style="color:#A6E22E">curl</span><span style="color:#AE81FF"> -L</span><span style="color:#E6DB74"> https://hydra.nixos.org/job/nix/maintenance-2.20/buildStatic.x86_64-linux/latest/download-by-type/file/binary-dist</span><span style="color:#F92672"> &gt;</span><span style="color:#E6DB74"> nixstatic</span></span>
<span class="line"><span style="color:#A6E22E">chmod</span><span style="color:#E6DB74"> +x</span><span style="color:#E6DB74"> nixstatic</span></span></code></pre></div>`,xe,O,yt=`By default, <code>nix</code> expects everything managed by <code>nix</code> related to be placed
...
by editing the <code>~/.config/nix/nix.conf</code> file:`,be,f,ft=`<div class="code-block-header">In File [~/.config/nix/nix.conf] <span class="code-block-lang">[plaintext]</span></div> <div class="code-block "><pre class="shiki monokai" style="background-color:#272822;color:#F8F8F2" tabindex="0"><code><span class="line"><span>store = &lt;your/store/path&gt;</span></span>
<span class="line"><span>extra-experimental-features = flakes nix-command</span></span>
<span class="line"><span>ssl-cert-file = /etc/pki/tls/cert.pem</span></span></code></pre></div>`,ke,W,vt=`(As of 2024 May, the experimental features are required, or you will be typing
<code>--extra-experimental-features</code> a lot). With this you can now spin up an
interactive shell that contain the actual nix commands:`,Fe,v,gt='<div class="code-block-header">command for activating shell with nix commands <span class="code-block-lang">[bash]</span></div> <div class="code-block code-copyable"><pre class="shiki monokai" style="background-color:#272822;color:#F8F8F2" tabindex="0"><code><span class="line"><span style="color:#A6E22E">./nixstatic</span><span style="color:#E6DB74"> shell</span><span style="color:#E6DB74"> nixpkgs#nix</span><span style="color:#E6DB74"> nixpkgs#git</span><span style="color:#E6DB74"> nixpkgs#bashInteractive</span><span style="color:#AE81FF"> --command</span><span style="color:#E6DB74"> bash</span></span></code></pre></div>',Te,V,wt=`You will notice that the first time you do this is very slow, because nix
...
stable version found in the <a href="https://search.nixos.org/packages" rel="nofollow">NixOS package repository</a>.`,He,z,xt=`You will also notice that subsequent calls to the spinning up this shell is now
...
download an instance of the package every time.`,Le,N,bt=`While it is tempting to stop here, and simply declare an alias that spins up
all your favorite tool with some alias:`,Me,g,kt='<div class="code-block-header">Do not do this <span class="code-block-lang">[bash]</span></div> <div class="code-block "><pre class="shiki monokai" style="background-color:#272822;color:#F8F8F2" tabindex="0"><code><span class="line"><span style="color:#F92672">&lt;</span><span style="color:#F8F8F2">path</span><span style="color:#F92672">&gt;</span><span style="color:#F8F8F2">/to/nixstatic shell nixpkgs#tool1 nixpkgs#tool2 ... --command bash</span></span></code></pre></div>',Ee,U,Ft=`This is missing out the full power of <code>nix</code>. The design of <code>nix</code> is to define
...
tool kit should be defined as just another dot file.`,_e,Y,Tt=`This declaration of the required tool kit can be done either globally for the
...
with.`,Ce,w,Ht='<a aria-hidden="true" tabindex="-1" href="#setting-up-home-manager"><span class="icon icon-link"></span></a>Setting up home-manager',De,G,Lt=`The nix-specific <a href="https://nix-community.github.io/home-manager/" rel="nofollow"><code>home-manager</code> package</a> is originally designed
...
effectively all the packages we are interested in.`,Ie,$,Mt=`The default distribution of home manager expects a standard nix set up where
...
minimally have nix, bash and an updated version of git installed:`,qe,x,Et=`<div class="code-block-header">run within nix shell <span class="code-block-lang">[bash]</span></div> <div class="code-block code-copyable"><pre class="shiki monokai" style="background-color:#272822;color:#F8F8F2" tabindex="0"><code><span class="line"><span style="color:#88846F">## Set up a nix shell with nix static, must include at least nix and an updated git</span></span>
...
<span class="line"><span style="color:#88846F">## create ~/.config/home-manager/flake.nix</span></span></code></pre></div>`,Ae,R,_t=`This will create the files required to declaring your default home environment.
...
<span class="line"><span style="color:#F8F8F2">}</span><span style="color:#F44747">;</span></span></code></pre></div>`,Be,J,Dt=`Once you are happy with the list of packages, you can run the following items
within a nix shells`,je,k,It=`<div class="code-block-header">run within nix shell <span class="code-block-lang">[bash]</span></div> <div class="code-block code-copyable"><pre class="shiki monokai" style="background-color:#272822;color:#F8F8F2" tabindex="0"><code><span class="line"><span style="color:#66D9EF">cd</span><span style="color:#E6DB74"> home-manager</span></span>
<span class="line"><span style="color:#A6E22E">./results/bin/home-manager</span><span style="color:#E6DB74"> switch</span></span></code></pre></div>`,Se,X,qt=`This will install all the programs that you are using to <code>$HOME/.nix-profile</code>
...
<span class="line"><span style="color:#F92672">export</span><span style="color:#F8F8F2"> PATH</span><span style="color:#F92672">=</span><span style="color:#F8F8F2">$HOME/.nix-profile/bin/:$PATH</span></span></code></pre></div>`,We,K,Pt=`Then you can jump all the way from the default login shell to your home-manager
defined environment using the following one-liner:`,Ve,T,Bt='<div class="code-block-header">run in default environment <span class="code-block-lang">[bash]</span></div> <div class="code-block code-copyable"><pre class="shiki monokai" style="background-color:#272822;color:#F8F8F2" tabindex="0"><code><span class="line"><span style="color:#A6E22E">./nixstatic</span><span style="color:#E6DB74"> shell</span><span style="color:#E6DB74"> nixpkgs#nix</span><span style="color:#AE81FF"> --command</span><span style="color:#E6DB74"> bash</span><span style="color:#AE81FF"> -l</span><span style="color:#AE81FF"> --rcfile=</span><span style="color:#F8F8F2">$HOME</span><span style="color:#AE81FF">/.bashrc-nix.sh</span></span></code></pre></div>',ze,Q,jt=`In this environment, you should be able to use all the packages that you have
...
reliability mantra of nix in full effect!)`,Ne,Z,St=`To automate the process of spinning up a nix shell with home-manager packages
when you log into the remote machine with a special tag, we can add an entry to
your local <code>ssh</code> configuration as something like:`,Ue,H,Ot=`<div class="code-block-header">~/.ssh/config <span class="code-block-lang">[plaintext]</span></div> <div class="code-block "><pre class="shiki monokai" style="background-color:#272822;color:#F8F8F2" tabindex="0"><code><span class="line"><span>Host remotehost*</span></span>
...
<span class="line"><span>    RemoteCommand &lt;path&gt;/to/nixstatic sheel nixpkgs#nix --command bash -l --rcfile=$HOME/.bashrc-nix.sh</span></span></code></pre></div>`,Ye,ee,Wt="Notice that the <code>RequestTTY</code> is required for the shell prompt.",Ge,L,Vt='<a aria-hidden="true" tabindex="-1" href="#some-limitations"><span class="icon icon-link"></span></a>Some limitations',$e,te,zt=`While this solution works wonderfully for my cases (I mainly just wanted to be
...
if you want to try this solution for yourself:`,Re,se,Nt=`<li><p>Given how <code>nix</code> aims to ensure package compatibility, the path you use to
...
environments with common tools, but that is beyond the scope of this article.</p></li>`,Je,M,Ut='<a aria-hidden="true" tabindex="-1" href="#some-closing-thoughts"><span class="icon icon-link"></span></a>Some closing thoughts',Xe,ne,Yt=`The declarative nix package manager has been on my radar for nearly 6 months
...
for real.`;return{c(){p=a("nav"),p.innerHTML=Qe,ae=i(),h=a("h2"),h.innerHTML=Ze,oe=i(),E=a("p"),E.innerHTML=et,le=i(),_=a("p"),_.innerHTML=tt,ie=i(),C=a("ul"),C.innerHTML=st,re=i(),d=a("h2"),d.innerHTML=nt,ce=i(),D=a("p"),D.textContent=at,pe=i(),I=a("ul"),I.innerHTML=ot,he=i(),q=a("ul"),q.innerHTML=lt,de=i(),A=a("ul"),A.innerHTML=it,ue=i(),P=a("p"),P.innerHTML=rt,me=i(),u=a("h2"),u.innerHTML=ct,ye=i(),B=a("p"),B.innerHTML=pt,fe=i(),j=a("p"),j.innerHTML=ht,ve=i(),m=a("h3"),m.innerHTML=dt,ge=i(),S=a("p"),S.innerHTML=ut,we=i(),y=a("div"),y.innerHTML=mt,xe=i(),O=a("p"),O.innerHTML=yt,be=i(),f=a("div"),f.innerHTML=ft,ke=i(),W=a("p"),W.innerHTML=vt,Fe=i(),v=a("div"),v.innerHTML=gt,Te=i(),V=a("p"),V.innerHTML=wt,He=i(),z=a("p"),z.innerHTML=xt,Le=i(),N=a("p"),N.textContent=bt,Me=i(),g=a("div"),g.innerHTML=kt,Ee=i(),U=a("p"),U.innerHTML=Ft,_e=i(),Y=a("p"),Y.innerHTML=Tt,Ce=i(),w=a("h3"),w.innerHTML=Ht,De=i(),G=a("p"),G.innerHTML=Lt,Ie=i(),$=a("p"),$.innerHTML=Mt,qe=i(),x=a("div"),x.innerHTML=Et,Ae=i(),R=a("p"),R.innerHTML=_t,Pe=i(),b=a("div"),b.innerHTML=Ct,Be=i(),J=a("p"),J.textContent=Dt,je=i(),k=a("div"),k.innerHTML=It,Se=i(),X=a("p"),X.innerHTML=qt,Oe=i(),F=a("div"),F.innerHTML=At,We=i(),K=a("p"),K.textContent=Pt,Ve=i(),T=a("div"),T.innerHTML=Bt,ze=i(),Q=a("p"),Q.innerHTML=jt,Ne=i(),Z=a("p"),Z.innerHTML=St,Ue=i(),H=a("div"),H.innerHTML=Ot,Ye=i(),ee=a("p"),ee.innerHTML=Wt,Ge=i(),L=a("h2"),L.innerHTML=Vt,$e=i(),te=a("p"),te.innerHTML=zt,Re=i(),se=a("ul"),se.innerHTML=Nt,Je=i(),M=a("h2"),M.innerHTML=Ut,Xe=i(),ne=a("p"),ne.textContent=Yt,this.h()},l(e){p=o(e,"NAV",{class:!0,"data-svelte-h":!0}),l(p)!=="svelte-x6c5t2"&&(p.innerHTML=Qe),ae=r(e),h=o(e,"H2",{id:!0,"data-svelte-h":!0}),l(h)!=="svelte-gst0r3"&&(h.innerHTML=Ze),oe=r(e),E=o(e,"P",{"data-svelte-h":!0}),l(E)!=="svelte-r6ptsw"&&(E.innerHTML=et),le=r(e),_=o(e,"P",{"data-svelte-h":!0}),l(_)!=="svelte-xhl2lc"&&(_.innerHTML=tt),ie=r(e),C=o(e,"UL",{"data-svelte-h":!0}),l(C)!=="svelte-12ov4f5"&&(C.innerHTML=st),re=r(e),d=o(e,"H2",{id:!0,"data-svelte-h":!0}),l(d)!=="svelte-l6p5gg"&&(d.innerHTML=nt),ce=r(e),D=o(e,"P",{"data-svelte-h":!0}),l(D)!=="svelte-1v1cfgo"&&(D.textContent=at),pe=r(e),I=o(e,"UL",{"data-svelte-h":!0}),l(I)!=="svelte-qx67gh"&&(I.innerHTML=ot),he=r(e),q=o(e,"UL",{"data-svelte-h":!0}),l(q)!=="svelte-l7pzx7"&&(q.innerHTML=lt),de=r(e),A=o(e,"UL",{"data-svelte-h":!0}),l(A)!=="svelte-hvuyr9"&&(A.innerHTML=it),ue=r(e),P=o(e,"P",{"data-svelte-h":!0}),l(P)!=="svelte-1y9kogl"&&(P.innerHTML=rt),me=r(e),u=o(e,"H2",{id:!0,"data-svelte-h":!0}),l(u)!=="svelte-10fhw2r"&&(u.innerHTML=ct),ye=r(e),B=o(e,"P",{"data-svelte-h":!0}),l(B)!=="svelte-zl8mbj"&&(B.innerHTML=pt),fe=r(e),j=o(e,"P",{"data-svelte-h":!0}),l(j)!=="svelte-1c2ke7c"&&(j.innerHTML=ht),ve=r(e),m=o(e,"H3",{id:!0,"data-svelte-h":!0}),l(m)!=="svelte-iss7kq"&&(m.innerHTML=dt),ge=r(e),S=o(e,"P",{"data-svelte-h":!0}),l(S)!=="svelte-d58n00"&&(S.innerHTML=ut),we=r(e),y=o(e,"DIV",{class:!0,"data-svelte-h":!0}),l(y)!=="svelte-2yhkcd"&&(y.innerHTML=mt),xe=r(e),O=o(e,"P",{"data-svelte-h":!0}),l(O)!=="svelte-1kg0ekw"&&(O.innerHTML=yt),be=r(e),f=o(e,"DIV",{class:!0,"data-svelte-h":!0}),l(f)!=="svelte-enh41a"&&(f.innerHTML=ft),ke=r(e),W=o(e,"P",{"data-svelte-h":!0}),l(W)!=="svelte-rjgsxi"&&(W.innerHTML=vt),Fe=r(e),v=o(e,"DIV",{class:!0,"data-svelte-h":!0}),l(v)!=="svelte-16uew0i"&&(v.innerHTML=gt),Te=r(e),V=o(e,"P",{"data-svelte-h":!0}),l(V)!=="svelte-c60zvr"&&(V.innerHTML=wt),He=r(e),z=o(e,"P",{"data-svelte-h":!0}),l(z)!=="svelte-1mpz4sd"&&(z.innerHTML=xt),Le=r(e),N=o(e,"P",{"data-svelte-h":!0}),l(N)!=="svelte-18yhdxy"&&(N.textContent=bt),Me=r(e),g=o(e,"DIV",{class:!0,"data-svelte-h":!0}),l(g)!=="svelte-1ugtrwm"&&(g.innerHTML=kt),Ee=r(e),U=o(e,"P",{"data-svelte-h":!0}),l(U)!=="svelte-1xauitj"&&(U.innerHTML=Ft),_e=r(e),Y=o(e,"P",{"data-svelte-h":!0}),l(Y)!=="svelte-lvfopj"&&(Y.innerHTML=Tt),Ce=r(e),w=o(e,"H3",{id:!0,"data-svelte-h":!0}),l(w)!=="svelte-9fyiyi"&&(w.innerHTML=Ht),De=r(e),G=o(e,"P",{"data-svelte-h":!0}),l(G)!=="svelte-1wost0m"&&(G.innerHTML=Lt),Ie=r(e),$=o(e,"P",{"data-svelte-h":!0}),l($)!=="svelte-1qvui6s"&&($.innerHTML=Mt),qe=r(e),x=o(e,"DIV",{class:!0,"data-svelte-h":!0}),l(x)!=="svelte-3cheqo"&&(x.innerHTML=Et),Ae=r(e),R=o(e,"P",{"data-svelte-h":!0}),l(R)!=="svelte-1o7i5vj"&&(R.innerHTML=_t),Pe=r(e),b=o(e,"DIV",{class:!0,"data-svelte-h":!0}),l(b)!=="svelte-1wpwb6y"&&(b.innerHTML=Ct),Be=r(e),J=o(e,"P",{"data-svelte-h":!0}),l(J)!=="svelte-fjke4n"&&(J.textContent=Dt),je=r(e),k=o(e,"DIV",{class:!0,"data-svelte-h":!0}),l(k)!=="svelte-1b9r55m"&&(k.innerHTML=It),Se=r(e),X=o(e,"P",{"data-svelte-h":!0}),l(X)!=="svelte-7pphin"&&(X.innerHTML=qt),Oe=r(e),F=o(e,"DIV",{class:!0,"data-svelte-h":!0}),l(F)!=="svelte-5gwj1e"&&(F.innerHTML=At),We=r(e),K=o(e,"P",{"data-svelte-h":!0}),l(K)!=="svelte-rg2wha"&&(K.textContent=Pt),Ve=r(e),T=o(e,"DIV",{class:!0,"data-svelte-h":!0}),l(T)!=="svelte-e07385"&&(T.innerHTML=Bt),ze=r(e),Q=o(e,"P",{"data-svelte-h":!0}),l(Q)!=="svelte-1348wse"&&(Q.innerHTML=jt),Ne=r(e),Z=o(e,"P",{"data-svelte-h":!0}),l(Z)!=="svelte-1qk3yj5"&&(Z.innerHTML=St),Ue=r(e),H=o(e,"DIV",{class:!0,"data-svelte-h":!0}),l(H)!=="svelte-j2qvhj"&&(H.innerHTML=Ot),Ye=r(e),ee=o(e,"P",{"data-svelte-h":!0}),l(ee)!=="svelte-1p5n920"&&(ee.innerHTML=Wt),Ge=r(e),L=o(e,"H2",{id:!0,"data-svelte-h":!0}),l(L)!=="svelte-19w07d7"&&(L.innerHTML=Vt),$e=r(e),te=o(e,"P",{"data-svelte-h":!0}),l(te)!=="svelte-1sqmc6l"&&(te.innerHTML=zt),Re=r(e),se=o(e,"UL",{"data-svelte-h":!0}),l(se)!=="svelte-133e0tp"&&(se.innerHTML=Nt),Je=r(e),M=o(e,"H2",{id:!0,"data-svelte-h":!0}),l(M)!=="svelte-h4m1ux"&&(M.innerHTML=Ut),Xe=r(e),ne=o(e,"P",{"data-svelte-h":!0}),l(ne)!=="svelte-1118fk0"&&(ne.textContent=Yt),this.h()},h(){c(p,"class","toc"),c(h,"id","what-is-the-problem"),c(d,"id","some-attempted-solutions"),c(u,"id","nix-as-a-portable-package-manager"),c(m,"id","nix-running-without-root"),c(y,"class","code-block-container"),c(f,"class","code-block-container"),c(v,"class","code-block-container"),c(g,"class","code-block-container"),c(w,"id","setting-up-home-manager"),c(x,"class","code-block-container"),c(b,"class","code-block-container"),c(k,"class","code-block-container"),c(F,"class","code-block-container"),c(T,"class","code-block-container"),c(H,"class","code-block-container"),c(L,"id","some-limitations"),c(M,"id","some-closing-thoughts")},m(e,t){s(e,p,t),s(e,ae,t),s(e,h,t),s(e,oe,t),s(e,E,t),s(e,le,t),s(e,_,t),s(e,ie,t),s(e,C,t),s(e,re,t),s(e,d,t),s(e,ce,t),s(e,D,t),s(e,pe,t),s(e,I,t),s(e,he,t),s(e,q,t),s(e,de,t),s(e,A,t),s(e,ue,t),s(e,P,t),s(e,me,t),s(e,u,t),s(e,ye,t),s(e,B,t),s(e,fe,t),s(e,j,t),s(e,ve,t),s(e,m,t),s(e,ge,t),s(e,S,t),s(e,we,t),s(e,y,t),s(e,xe,t),s(e,O,t),s(e,be,t),s(e,f,t),s(e,ke,t),s(e,W,t),s(e,Fe,t),s(e,v,t),s(e,Te,t),s(e,V,t),s(e,He,t),s(e,z,t),s(e,Le,t),s(e,N,t),s(e,Me,t),s(e,g,t),s(e,Ee,t),s(e,U,t),s(e,_e,t),s(e,Y,t),s(e,Ce,t),s(e,w,t),s(e,De,t),s(e,G,t),s(e,Ie,t),s(e,$,t),s(e,qe,t),s(e,x,t),s(e,Ae,t),s(e,R,t),s(e,Pe,t),s(e,b,t),s(e,Be,t),s(e,J,t),s(e,je,t),s(e,k,t),s(e,Se,t),s(e,X,t),s(e,Oe,t),s(e,F,t),s(e,We,t),s(e,K,t),s(e,Ve,t),s(e,T,t),s(e,ze,t),s(e,Q,t),s(e,Ne,t),s(e,Z,t),s(e,Ue,t),s(e,H,t),s(e,Ye,t),s(e,ee,t),s(e,Ge,t),s(e,L,t),s(e,$e,t),s(e,te,t),s(e,Re,t),s(e,se,t),s(e,Je,t),s(e,M,t),s(e,Xe,t),s(e,ne,t)},p:Ke,i:Ke,o:Ke,d(e){e&&(n(p),n(ae),n(h),n(oe),n(E),n(le),n(_),n(ie),n(C),n(re),n(d),n(ce),n(D),n(pe),n(I),n(he),n(q),n(de),n(A),n(ue),n(P),n(me),n(u),n(ye),n(B),n(fe),n(j),n(ve),n(m),n(ge),n(S),n(we),n(y),n(xe),n(O),n(be),n(f),n(ke),n(W),n(Fe),n(v),n(Te),n(V),n(He),n(z),n(Le),n(N),n(Me),n(g),n(Ee),n(U),n(_e),n(Y),n(Ce),n(w),n(De),n(G),n(Ie),n($),n(qe),n(x),n(Ae),n(R),n(Pe),n(b),n(Be),n(J),n(je),n(k),n(Se),n(X),n(Oe),n(F),n(We),n(K),n(Ve),n(T),n(ze),n(Q),n(Ne),n(Z),n(Ue),n(H),n(Ye),n(ee),n(Ge),n(L),n($e),n(te),n(Re),n(se),n(Je),n(M),n(Xe),n(ne))}}}const Zt={title:"Nix as a local package manager",description:"A first exploration for carrying your personal development tool everywhere",tags:["vim","computing","tips"],banner:"thoughts.jpg"};class es extends Rt{constructor(p){super(),Jt(this,p,null,Xt,$t,{})}}export{es as default,Zt as metadata};
